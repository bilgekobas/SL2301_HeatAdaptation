...
Outputs (sex-adjusted primary models):
...
# By default, the script looks for input files next to this .py file.
# You can also replace these paths with your local Windows paths if preferred.
BASE_DIR = Path(__file__).resolve().parent
PATH_HRV  = str(BASE_DIR / "02_AllHRV_withSed.csv")
PATH_PART = str(BASE_DIR / "00_Participants_Meta.xlsx")

OUTDIR = str(BASE_DIR / "HRV_outputs_sex_adjusted")
...
def normalize_bool_series(x: pd.Series) -> pd.Series:
    ...
    if s.dtype == object:
        ...
        is_true = ss.isin(["true", "1", "yes", "y", "t", "ok"])
...
def load_and_prepare() -> tuple[pd.DataFrame, pd.DataFrame]:
    ...
    # datetime, part_id, condition (arm), scenario, sedentary/is_ok_sedentary, bpm_mean, hrv_rmssd, hrv_hf
    if "sedentary" not in df.columns and "is_ok_sedentary" in df.columns:
        df = df.rename(columns={"is_ok_sedentary": "sedentary"})
...
def model_selection(drift_df: pd.DataFrame) -> pd.DataFrame:
    """
    Compare sex-adjusted candidate models via AIC/BIC.

    Sex is retained a priori in all final physiological models. Therefore, the
    primary base model is arm × scenario + sex_c, and additional models test
    whether fat percentage improves fit beyond this sex-adjusted base.
    """
    formulas = [
    ...
    if out["bic"].notna().any():
        out["delta_bic_vs_best"] = out["bic"] - out["bic"].min()
    else:
...
def fit_final(drift_df: pd.DataFrame, ms: pd.DataFrame):
    # Final model is selected only from sex-adjusted candidates.
    # Fallback also retains sex_c a priori.
